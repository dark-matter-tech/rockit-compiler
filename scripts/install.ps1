...
#
# Usage (PowerShell):
#   irm https://rustygits.com/Dark-Matter/rockit-compiler/raw/branch/master/scripts/install.ps1 | iex
#
# Or run locally:
#   .\install.ps1
...
# Signing key URL
$SIGNING_KEY_URL = "$GITEA/$REPO/raw/branch/master/keys/darkmatter-release.asc"
...
if ($downloaded) {
    ...
    if (Test-Path "$tmp\moon\RockitCompiler\src\stdlib") {
        Copy-Item "$tmp\moon\RockitCompiler\src\stdlib" "$SHARE_DIR\stdlib" -Recurse -Force
    }
